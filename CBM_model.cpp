...
{
	/*VideoWriter mm("ddd.avi", VideoWriter::fourcc(), 30, cv::Size(new_width, new_height));
	_writer1("long.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 30, cv::Size(new_width, new_height));
	_writer2("short.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 30, cvSize(new_width, new_height), 0);
	_writer3("static.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 30, cvSize(new_width, new_height), 1);
	_writer4("DPM.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 30, cvSize(new_width, new_height), 1);*/
...
bool CBM_model::Motion_Detection(myImage *img)
{
	...
	if( frame_count < MOG_LEARN_FRAMES){
		...
		cv::Mat mog_fg_mat = cvarrToMat(mog_fg);
		cv::Mat mog_fg2_mat = cvarrToMat(mog_fg2);
		cv::Mat imgStatic_mat = cvarrToMat(imgStatic);

		/*_writer1.write(mog_fg_mat);
		_writer2.write(mog_fg2_mat);
		_writer3.write(imgStatic_mat);*/
		return false;
	}
	else{
...
void CBM_model::DetectPrevious_nForeground_DPM2( int n)
{
	...
	if (foregournd_found == true)
	{
		...
		for (int i = 0; i < object_num; i++)
		{
			...
			//using opencv4's people detections as as substitute for opencv2 svm functions
			// Iplimage* converts to cv::Mat
			cv::Mat frame = cvarrToMat(temp);
			vector<Rect> found = m_detector.detect(frame);

			for (vector<Rect>::iterator i = found.begin(); i != found.end(); ++i)
			{
				Rect& r = *i;
				m_detector.adjustRect(r);
				rectangle(frame, r.tl(), r.br(), cv::Scalar(0, 255, 0), 2);
				printf("x = %d, y %d, w = %d, h = %d\n", roi_x + r.x, roi_y + r.y, r.width, r.height);
				if ((r.x > 0) && (r.x < new_width) && (r.x + r.width > 0) && (r.x + r.width < new_width) &&
					(r.y > 0) && (r.y < new_height) && (r.y + r.height > 0) && (r.y + r.height < new_height) &&
					(r.width * r.height < MAX_FG) && (r.height > r.width))
				{
					for (int x = roi_x + r.x; x <= (roi_x + r.x + r.width); x++) {
						for (int y = roi_y + r.y; y <= (roi_y + r.y + r.height); y++) {
			...
			/*for( int i = 0; i < detections->total; i++ )
			{
				...
				if (score>0)
				{
					CvRect r = detection.rect;

					cvRectangle( temp, cvPoint(r.x, r.y),
									cvPoint(r.x + r.width,
									r.y + r.height),
...
{
	//cv::Mat temp(cv::Size(new_width, new_height), CV_8UC3);
	cv::Mat gray(cv::Size(new_width, new_height), CV_8UC1);
	IplImage * temp = cvCreateImage(cvSize(new_width, new_height), IPL_DEPTH_8U, 3);
	myImage_2_opencv( _Previous_Img[(FG_count+(TEMPORAL_RULE-n))%TEMPORAL_RULE], temp);	
	cv::Mat mat_temp = cvarrToMat(temp);
	cv::cvtColor(mat_temp, gray, CV_RGB2GRAY);
    ...
    {
		Rect r = found[i];
		rectangle(mat_temp, r.tl(), r.br(), cv::Scalar(0, 255, 0), 2);
			printf("x = %d, y %d, w = %d, h = %d\n",r.x,r.y,r.width,r.height);
			if ((r.x>0)&&(r.x<new_width)&&(r.x+r.width>0)&&(r.x+r.width<new_width)&&
				(r.y>0)&&(r.y<new_height)&&(r.y+r.height>0)&&(r.y+r.height<new_height))
			{
